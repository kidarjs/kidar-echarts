...
export const setZoom = (barsWidth, ctx) => {
    const zoom = {};
    ...
    zoom.endValue = end;
    zoom.zoomLock = true;
    zoom.brushSelect = false;
    zoom.height = 10;
    return zoom;
};
export function omitNum(val) {
    if (!Number(val)) {
        return 0;
    }
    let res = '';
 ...
 * @param val
 */
export function approximateNum(val) {
    if (!Number(val)) {
        return 0;
    }
    let res = val;
...
export function getLinearColor(startColor, endColor) {
    const areaStyle = {
        color: {
            type: 'linear',
            x: 0,
